...
import com.example.cloudinaction.exceptions.CategoryNotFoundException;
import com.example.cloudinaction.exceptions.InvalidStateException;
...
public class DefaultCategoryService implements CategoryService {
    ...
    private static final String CATEGORY_ALREADY_CONTAINS_PRODUCT = "Category id: [%s] already contains product id: [%s]";
    ...
    public void deleteCategory(Long id) {
        Category category = categoryRepository.findById(id)
                .orElseThrow(() -> new CategoryNotFoundException(String.format(CATEGORY_NOT_FOUND_MESSAGE, ID_FIELD, id)));
    ...
    public Category assignProductToCategory(Long catId, Long prdId) {
        Product targetProduct = productRepository.findById(prdId)
                .orElseThrow(() -> new CategoryNotFoundException(String.format(PRODUCT_NOT_FOUND_MESSAGE, ID_FIELD, prdId)));

        Category targetCategory = categoryRepository.findById(catId)
                .orElseThrow(() -> new CategoryNotFoundException(String.format(CATEGORY_NOT_FOUND_MESSAGE, ID_FIELD, catId)));

        boolean contains = targetCategory.getProducts().contains(targetProduct);

        if (contains) {
            throw new InvalidStateException(String.format(CATEGORY_ALREADY_CONTAINS_PRODUCT, catId, prdId));
        }
    ...
    public Category unAssignProductToCategory(Long catId, Long prdId) {
        ...
        if (!contains) {
            throw new InvalidStateException(String.format(CATEGORY_IS_NOT_CONTAINS_PRODUCT, catId, prdId));
        }
